...
export class KnowRuleComponent {
  ...
  private selectQuestion(): void {
    this.generateItem();
    this.textoPergunta = `"${this.itens[this.itemAtual].item}" passa pelo túnel invisível?`
  }

  public tryAnswer(value: boolean): void {
    if (value === this.itens[this.itemAtual].passa){
      this.corretAnswer();
    } else {
      this.wrongAnswer();
    }
  ...
  public share(): void {
    HtmlUtils.share(
      "Jogo do Túnel Invisível",
      "Eu consegui descobrir a regra do jogo do Túnel Invisível! E você, consegue?",
